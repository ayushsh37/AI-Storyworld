...
def create_world(req: CreateWorldReq):
    wid = str(uuid.uuid4())
    state = {"id": wid, "name": req.name, "time": 0, "locations": [], "global_facts": [], "agents": [], "agents_map": {}, "event_log": []}
    conn = sqlite3.connect(os.getenv("STORY_DB", "storyworld.db"))
    c = conn.cursor()
    c.execute("INSERT INTO worlds (id,state,created_at) VALUES (?,?,?)", (wid, json.dumps(state), datetime.datetime.utcnow().isoformat()))
...
def register_agent(req: RegisterAgentReq, world_id: str):
    ...
    conn = sqlite3.connect(os.getenv("STORY_DB", "storyworld.db"))
    c = conn.cursor()
    c.execute("INSERT INTO agents (id,name,endpoint,world_id,type) VALUES (?,?,?,?,?)", (aid, req.name, req.endpoint, world_id, req.type))
    conn.commit(); conn.close()
    world = load_world(world_id)
    if world:
        world["agents"].append({"id": aid, "type": req.type, "role": req.name, "state": {}})
        world["agents_map"][aid] = {}
        persist_world(world_id, world)
    return {"id": aid, "name": req.name, "endpoint": req.endpoint, "type": req.type}
...
def get_events(world_id: str):
    conn = sqlite3.connect(os.getenv("STORY_DB", "storyworld.db"))
    c = conn.cursor()
    c.execute("SELECT event_json, created_at FROM events WHERE world_id=? ORDER BY created_at", (world_id,))
